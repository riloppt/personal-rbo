...
export const qrUrl = t =>
  `https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(t)}`;

export const calcDuration = (hora_inicio, hora_fim) => {
...
export const buildReportHtml = ({ mov, cliente, tipologia, tecnico, local }) => {
  const duracao = calcDuration(mov.hora_inicio, mov.hora_fim);
...
// ─── Pure business-logic helpers (no side effects) ────────────────────────────

/** Sum credits across a list of movimento objects */
export const calcSaldo = movimentos =>
  movimentos.reduce((sum, m) => sum + m.creditos, 0);

/** Aggregate per-contract credit stats and last-assistencia date from movements */
export const buildMovStats = movimentos => {
  const stats = {};
  movimentos.forEach(m => {
    if (!stats[m.contrato_id]) stats[m.contrato_id] = { creditos: 0, ultimaAssist: null };
    stats[m.contrato_id].creditos += m.creditos;
    if (m.tipo === "assistencia" && !stats[m.contrato_id].ultimaAssist) {
      stats[m.contrato_id].ultimaAssist = m.data;
    }
  });
  return stats;
};

/** Filter contracts list by a search query against client/tipologia names */
export const filterContratos = (rows, clientes, tipologias, search) => {
  if (!search) return rows;
  const q = search.toLowerCase();
  return rows.filter(c => {
    const cli = clientes.find(x => x.id === c.cliente_id);
    const tip = tipologias.find(x => x.id === c.tipologia_id);
    return cli?.nome.toLowerCase().includes(q) || tip?.nome.toLowerCase().includes(q);
  });
};
