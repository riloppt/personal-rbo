    ...
    <>
      ...
      {hasMore && (
        <div style={{
          ...
          background:C.tealD, borderRadius:"20px 20px 0 0",
          ...
          <div style={{padding:"8px 0 4px"}}>
            {overflow.map(item=>{
              ...
              return (
                <button key={item.id} onClick={()=>go(item.id)}
                  style={{width:"100%",display:"flex",alignItems:"center",gap:14,padding:"13px 24px",background:active?`${C.teal}33`:"transparent",border:"none",cursor:"pointer",borderLeft:active?"3px solid #fff":"3px solid transparent",transition:"all .15s"}}>
                  <Icon name={item.icon} size={20} color={active?"#ffffff":C.tealL}/>
                  <span style={{fontSize:15,fontWeight:active?600:400,color:active?"#ffffff":C.tealL}}>{item.label}</span>
                </button>
              );
            })}
            {/* Dark mode no drawer */}
            <button onClick={()=>{onToggleDark();setDrawerOpen(false);}}
              ...
              <Icon name={darkMode?"sun":"moon"} size={20} color={C.tealL}/>
              <span style={{fontSize:15,color:C.tealL}}>{darkMode?"Modo claro":"Modo escuro"}</span>
            </button>
            {/* Logout */}
      ...
      <nav style={{
        ...
        {primary.map(item=>{
          ...
          return (
            <button key={item.id} onClick={()=>go(item.id)}
              ...
              <Icon name={item.icon} size={20} color={active?"#ffffff":C.sidebarFg}/>
              <span style={{fontSize:10,fontWeight:active?700:400,color:active?"#ffffff":C.sidebarFg,letterSpacing:".2px"}}>{item.label}</span>
            </button>
          );
        })}
        ...
        {hasMore && (
          <button onClick={()=>setDrawerOpen(d=>!d)}
            ...
            <Icon name="menu" size={20} color={(moreActive||drawerOpen)?"#ffffff":C.sidebarFg}/>
            <span style={{fontSize:10,fontWeight:(moreActive||drawerOpen)?700:400,color:(moreActive||drawerOpen)?"#ffffff":C.sidebarFg,letterSpacing:".2px"}}>Mais</span>
          </button>
        )}
        ...
        {!hasMore && (
          <button onClick={onToggleDark}
            ...
            <Icon name={darkMode?"sun":"moon"} size={20} color={C.sidebarFg}/>
            <span style={{fontSize:10,color:C.sidebarFg,letterSpacing:".2px"}}>{darkMode?"Claro":"Escuro"}</span>
          </button>
        )}
